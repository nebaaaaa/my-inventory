...
// Simple {n}-placeholder substitution so a template can live in the i18n
// dictionary while still taking a dynamic number (days, qty, etc.).
function tf(key, n) {
    return t(key).replace('{n}', n);
}

function badge(days) {
    if (days < 0) return `<span style="background:#FAECE7; color:var(--danger); font-size:11px; padding:2px 8px; border-radius:20px; white-space:nowrap;">${tf('pharm.expired_ago', Math.abs(days))}</span>`;
    const danger = days <= DANGER_DAYS;
    const bg = danger ? '#FAECE7' : '#FCEEDD';
    const fg = danger ? 'var(--danger)' : 'var(--warning)';
    return `<span style="background:${bg}; color:${fg}; font-size:11px; padding:2px 8px; border-radius:20px; white-space:nowrap;">${tf('pharm.days_left', days)}</span>`;
}
    ...
    return `
        <div style="padding:10px 12px; background:${rowBg}; border-bottom:1px solid var(--border);">
            <div style="display:flex; justify-content:space-between; align-items:baseline; gap:8px;">
                ...
                <span style="font-family:'IBM Plex Mono',monospace; font-size:11px; color:var(--text-light); white-space:nowrap;">${tf('pharm.qty', r.lot.qty)}</span>
            </div>
            <div style="display:flex; justify-content:space-between; align-items:center; gap:8px; margin-top:6px; flex-wrap:wrap;">
                ...
                <div style="display:flex; align-items:center; gap:6px;">
                    ${badge(r.days)}
                    <button onclick="writeOffExpiringLot(${r.itemIndex}, '${r.lot.id}')" style="background:var(--surface); border:1px solid var(--border); border-radius:6px; padding:2px 8px; font-size:11px; cursor:pointer; color:var(--text-light); white-space:nowrap;">${t('pharm.write_off')}</button>
                </div>
            </div>
        </div>`;
}
...
function render(mountEl, state) {
    ...
    mountEl.innerHTML =
        cardHTML('pharm-soon', t('pharm.expiring_soon'), 'var(--danger)', soon, tf('pharm.nothing_expiring', WARN_DAYS)) +
        cardHTML('pharm-expired', t('pharm.expired'), 'var(--danger)', expired, t('pharm.no_expired_stock'));
