...
const t = (...a) => window.t(...a);
// Simple placeholder substitution so a translated template string can still
// carry dynamic values -- {a}, {b}, {c}... are replaced in order given.
function tf(key, ...vals) {
    let s = t(key);
    vals.forEach((v, i) => { s = s.split(`{${String.fromCharCode(97 + i)}}`).join(v); });
    return s;
}
...
function rateTypeLabel(rt) { return rt ? t('rental.rate.' + rt) : '—'; }
...
function statusBadge(status) {
    const map = {
        reserved: ['#DDE6F5', 'var(--info)', t('rental.status.reserved')],
        out: ['#FCEEDD', 'var(--warning)', t('rental.status.out')],
        overdue: ['#FAECE7', 'var(--danger)', t('rental.status.overdue')],
        returned: ['#E2EFE8', 'var(--success)', t('rental.status.returned')]
    };
...
export async function renderInventorySection(c) {
    ctx = { ...ctx, ...c };
    if (!ctx.state.rentalBookings) { ctx.mountEl.innerHTML = `<div class="card"><p style="color:var(--text-light);">${t('common.loading')}</p></div>`; await fetchBookings(); }
...
function renderDashboardWidget() {
    ...
    widgetMountEl.innerHTML = `
        <div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:8px;">
            ${miniStat(t('rental.out'), outCount, 'var(--warning)', 'out')}
            ${miniStat(t('rental.overdue'), overdueCount, 'var(--danger)', 'overdue')}
            ${miniStat(tf('rental.due_in_days', 3), upcoming, 'var(--info)', 'due_soon')}
        </div>`;
}
function miniStat(label, value, color, filterKey) {
    return `<div style="flex:1; min-width:100px; background:var(--surface); border:1px solid var(--border); border-radius:10px; padding:10px 12px; box-shadow:var(--shadow-sm); cursor:pointer;" onclick="rentalUI.openStatusPopup('${filterKey}', '${label}')" title="${t('rental.view_bookings')}">
...
function openStatusPopup(filterKey, label) {
    ...
    let html = '';
    if (matches.length === 0) {
        html = `<p style="color:var(--text-light); font-size:13px;">${t('rental.no_bookings_category')}</p>`;
    } else {
        html = `<table><thead><tr><th>${t('rental.customer')}</th><th>${t('rental.asset')}</th><th class="num">${t('tr.quantity')}</th><th>${t('rental.start')}</th><th>${t('rental.expected_return')}</th><th style="text-align:center;">${t('ch.status')}</th></tr></thead><tbody>
            ${matches.map(b => {
...
function renderInventoryContent() {
    const tabs = [['availability', t('rental.availability')], ['assets', t('rental.assets')]];
...
function filterPanelHTML() {
    return `
        <div class="filter-panel" style="margin-bottom:12px;">
            <div class="form-group"><label>${t('rental.search_category')}</label><input type="text" id="rental-inv-filter-category" placeholder="${t('rental.filter_category_ph')}" value="${inventoryFilterCategory}" oninput="rentalUI.applyInventoryFilter()"></div>
            <div class="form-group"><label>${t('rental.search_name')}</label><input type="text" id="rental-inv-filter-name" placeholder="${t('rental.filter_name_ph')}" value="${inventoryFilterName}" oninput="rentalUI.applyInventoryFilter()"></div>
        </div>`;
}
    ...
    return `
        <div class="card">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; flex-wrap:wrap; gap:10px;">
                <h2 style="border:none; margin:0;">${t('rental.availability')}</h2>
                <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                    <input type="date" id="cal-filter-date" value="${todayISO()}" onchange="rentalUI.jumpCalendarToDate()" style="font-size:12px;">
                    <button class="btn btn-small" style="background:var(--bg); color:var(--text); border:1px solid var(--border);" onclick="rentalUI.shiftCalendar(-5)">${t('rental.earlier')}</button>
                    <button class="btn btn-small" style="background:var(--bg); color:var(--text); border:1px solid var(--border);" onclick="rentalUI.shiftCalendar(5)">${t('rental.later')}</button>
                </div>
            </div>
            ${filterPanelHTML()}
...
function renderAvailabilityBody() {
    ...
    el.innerHTML = list.length === 0 ? `<p style="color:var(--text-light); font-size:13px;">${items().length === 0 ? t('rental.add_product_hint') : t('rental.no_assets_match_filter')}</p>` : `
        ...
        <div class="table-scroll" id="rental-availability-scroll" style="border:1px solid var(--border); border-radius:8px;">
            <table style="margin:0;">
                <thead>
                    <tr>
                        <th style="position:sticky; left:0; z-index:3; min-width:${ASSET_COL_WIDTH}px; height:36px; background:var(--bg);">${t('rental.asset')}</th>
...
function calendarCell(item, dateIso, canBook) {
    ...
    return `<td title="${tf('rental.avail_of_total', avail, total, fmtDate(dateIso))}"${onclick}><div style="width:100%; height:22px; background:${bg}; border-radius:4px; display:flex; align-items:center; justify-content:center; font-size:10px; color:var(--text-light);">${total > 0 ? avail : ''}</div></td>`;
}
...
function renderAssetsBody() {
    ...
    if (!el) return;
    const list = filteredItems();
    el.innerHTML = list.length === 0 ? `<p style="color:var(--text-light); font-size:13px;">${items().length === 0 ? t('rental.no_assets_yet') : t('rental.no_assets_match_filter')}</p>` : `
        <div class="table-scroll" style="border:1px solid var(--border); border-radius:8px;">
        <table style="margin:0;">
            <thead><tr><th>${t('inv.category')}</th><th>${t('inv.item_spec')}</th><th class="num">${t('tr.quantity')}</th><th class="num">${t('rental.rate.daily')}</th><th class="num">${t('rental.rate.weekly')}</th><th class="num">${t('rental.rate.monthly')}</th><th style="text-align:center;">${t('common.action')}</th></tr></thead>
            <tbody>
                ${list.map(i => `<tr>
                    <td>${i.category}</td>
                    ...
                    <td style="text-align:center; white-space:nowrap;">
                        ${hasPerm('inventory.edit_products') ? `<button class="btn btn-small" onclick="initiateEditProductDefinition(${itemIndexById(i.id)})">${t('common.edit')}</button> ` : ''}
                        ${hasPerm('rentals.write_off') && i.lots && i.lots.length > 0 ? `<button class="btn btn-small btn-danger" onclick="rentalUI.openWriteOffModal('${i.id}')">${t('rental.write_off')}</button>` : ''}
                    </td>
                </tr>`).join('')}
            </tbody>
        </table>
...
function openWriteOffModal(itemId) {
    ...
    <div class="modal-overlay active" onclick="rentalUI.closeWriteOffModal()">
        <div class="modal-box" onclick="event.stopPropagation()" style="max-width:460px;">
            <h3 style="color:var(--primary); margin-bottom:8px;">${tf('rental.write_off_title', item.desc)}</h3>
            <p style="font-size:13px; color:var(--text-light); margin-bottom:14px;">${t('rental.writeoff_modal_hint')}</p>
            ${(item.lots || []).length === 0 ? `<p style="color:var(--text-light); font-size:13px;">${t('rental.no_stock_writeoff')}</p>` : `
            <table>
                <thead><tr><th class="num">${t('tr.quantity')}</th><th class="num">${t('rental.unit_cost')}</th><th class="num" style="min-width:70px;">${t('rental.write_off')}</th><th style="text-align:center;">${t('common.action')}</th></tr></thead>
                <tbody>
                    ${item.lots.map(l => `<tr>
                        ...
                        <td style="text-align:center;"><button class="btn btn-small btn-danger" onclick="rentalUI.confirmWriteOff(${idx}, '${l.id}')">${t('rental.write_off')}</button></td>
                    </tr>`).join('')}
                </tbody>
            </table>`}
            <div style="display:flex; justify-content:flex-end; margin-top:14px;">
                <button class="btn btn-small" style="background:var(--bg); color:var(--text); border:1px solid var(--border);" onclick="rentalUI.closeWriteOffModal()">${t('common.close')}</button>
            </div>
        </div>
    </div>`;
}
...
function bookingsHTML() {
    const filters = [['active', t('rental.filter_active')], ['overdue', t('rental.status.overdue')], ['returned', t('rental.status.returned')], ['all', t('rental.filter_all')]];
    return `
        ${hasPerm('sales.create') ? newBookingInlineHTML() : ''}
        <div class="card">
            <h2>${t('rental.bookings_title')}</h2>
            ...
            <div class="filter-panel">
                <div class="form-group"><label>${t('sale.filter_date')}</label><input type="date" id="rbk-filter-date" value="${bookingsFilterDate}" onchange="rentalUI.applyBookingsFilter()"></div>
                <div class="form-group"><label>${t('rental.customer_name_phone')}</label><input type="text" id="rbk-filter-customer" placeholder="${t('common.search_ph')}" value="${bookingsFilterCustomer}" oninput="rentalUI.applyBookingsFilter()"></div>
                <div class="form-group"><label>${t('rental.product_item')}</label><input type="text" id="rbk-filter-item" placeholder="${t('common.search_ph')}" value="${bookingsFilterItem}" oninput="rentalUI.applyBookingsFilter()"></div>
                <div class="form-group"><label>${t('rental.start')}</label><input type="date" id="rbk-filter-start" value="${bookingsFilterStart}" onchange="rentalUI.applyBookingsFilter()"></div>
                <div class="form-group"><label>${t('sale.channel')}</label>
                    <select id="rbk-filter-channel" onchange="rentalUI.applyBookingsFilter()">
                        <option value="">${t('sale.all')}</option>
                        <option value="Cash" ${bookingsFilterChannel === 'Cash' ? 'selected' : ''}>${t('rental.cash')}</option>
                        <option value="Mobile banking" ${bookingsFilterChannel === 'Mobile banking' ? 'selected' : ''}>${t('rental.mobile_banking')}</option>
                        <option value="Accounts receivable" ${bookingsFilterChannel === 'Accounts receivable' ? 'selected' : ''}>${t('rental.accounts_receivable')}</option>
                    </select>
                </div>
                <div class="form-group" id="rbk-filter-by-group" style="display:${ctx.state.hasEmployees ? 'block' : 'none'};"><label>${t('common.by')}</label>
                    <select id="rbk-filter-author" onchange="rentalUI.applyBookingsFilter()"><option value="">${t('sale.all')}</option></select>
                </div>
            </div>
...
function syncBookingAuthorFilter() {
    ...
    sel.innerHTML = `<option value="">${t('sale.all')}</option>` + authors.map(a => `<option value="${a}">${a}</option>`).join('');
...
function renderBookingsTableBody() {
    ...
    if (totalEl) totalEl.innerText = tf('rental.total_shown', totalShown.toFixed(2), displayList.length);

    el.innerHTML = displayList.length === 0 ? `<p style="color:var(--text-light); font-size:13px;">${t('rental.no_bookings_filter')}</p>` : `
            <table>
                <thead><tr><th>${t('common.date')}</th><th>${t('sale.customer_name')}</th><th>${t('rental.product_item')}</th><th class="num">${t('rental.quantity')}</th><th>${t('rental.start')}</th><th>${t('rental.expected_return')}</th><th>${t('ch.status')}</th><th class="num">${t('ch.amount')}</th><th>${t('rental.channel_status_type')}</th><th class="branch-col">${t('common.branch')}</th><th>${t('common.by')}</th><th style="text-align:center;">${t('common.action')}</th></tr></thead>
...
function lineActions(b) {
    ...
    if (b.status === 'reserved') {
        if (hasPerm('sales.create')) actions += `<button class="btn btn-small" onclick="rentalUI.checkOutBooking('${b.id}')">${t('rental.check_out')}</button> `;
        if (hasPerm('sales.void')) actions += `<button class="btn btn-small btn-danger" onclick="rentalUI.cancelBooking('${b.id}')">${t('common.cancel')}</button> `;
    } else {
        ...
        if (b.status === 'out' && hasPerm('rentals.checkin')) actions += `<button class="btn btn-small" onclick="rentalUI.openCheckInModal('${b.id}')">${t('rental.check_in')}</button> `;
        if (hasPerm('sales.void')) actions += `<button class="btn btn-small btn-danger" onclick="rentalUI.voidBooking('${b.id}')">${t('rental.void')}</button> `;
    }
    return actions;
}
...
function voidBooking(id) {
    ...
    if (!b) return;
    if (isBranchClosed(b.branchId)) { alert(tf('rental.alert_branch_closed_void', branchNameById(b.branchId))); return; }
    const item = itemById(b.itemId);
    if (!confirm(tf('rental.confirm_void_booking', b.qty, itemLabel(item), b.customerName))) return;
    ...
    addNotification(t('rental.notif_booking_voided_title'), tf('rental.notif_booking_voided_body', itemLabel(item), b.customerName), tf('rental.notif_booking_voided_sub', displayStatus(b), b.amount.toFixed(2)));
    renderBookingContent();
}
...
function voidBookingGroup(groupId) {
    ...
    if (rows.some(r => isBranchClosed(r.branchId))) { alert(t('rental.alert_branch_closed_void_group')); return; }
    if (!confirm(tf('rental.confirm_void_group', rows.length, rows[0].customerName))) return;
    ...
    addNotification(t('rental.notif_booking_voided_title'), tf('rental.notif_group_voided_body', rows.length, rows[0].customerName), t('rental.notif_group_voided_sub'));
    renderBookingContent();
}
...
function channelStatusCell(record, total, rateType) {
    ...
    if (remaining <= 0.001) html += `<br><small style="color:var(--success)">${t('rental.paid_in_full_tag')}</small>`;
    else if ((record.amountPaid || 0) > 0) html += `<br><small style="color:var(--warning)">${tf('rental.owes_tag', remaining.toFixed(2))}</small>`;
    else html += `<br><small style="color:var(--danger)">${t('rental.pending_tag')}</small>`;
...
function bookingRow(b) {
    ...
    if (balance > 0.001 && hasPerm('sales.mark_paid')) actions += `<button class="btn btn-small" style="background:var(--bg); color:var(--text); border:1px solid var(--border);" onclick="rentalUI.openPaymentModal('${b.id}')">${t('common.record_payment')}</button> `;
    return `<tr>
...
function lineActionsForGroupChild(b) {
    if (b.status === 'out' && hasPerm('rentals.checkin')) return `<button class="btn btn-small" onclick="rentalUI.openCheckInModal('${b.id}')">${t('rental.check_in')}</button>`;
    return '';
}
...
function groupTransactionActions(groupId, rows, balance) {
    ...
    let html = '';
    if (balance > 0.001 && hasPerm('sales.mark_paid')) html += `<button class="btn btn-small" style="background:var(--bg); color:var(--text); border:1px solid var(--border);" onclick="rentalUI.openPaymentModal('${groupId}', true)">${t('common.record_payment')}</button> `;
    if (reserved.length > 0) {
        if (hasPerm('sales.create')) html += `<button class="btn btn-small" onclick="rentalUI.checkOutBookingGroup('${groupId}')">${t('rental.check_out')}</button> `;
        if (hasPerm('sales.void')) html += `<button class="btn btn-small btn-danger" onclick="rentalUI.cancelBookingGroup('${groupId}')">${t('common.cancel')}</button> `;
    }
    if (progressed && hasPerm('sales.void')) html += `<button class="btn btn-small btn-danger" onclick="rentalUI.voidBookingGroup('${groupId}')">${t('rental.void')}</button> `;
    return html;
}
...
function printBookingButtons(id, isGroup) {
    return `<button class="btn btn-small" style="background:var(--bg); color:var(--text); border:1px solid var(--border);" onclick="rentalUI.printBookingReceipt('${id}', ${!!isGroup})" title="${t('rental.print_receipt_title')}">${t('rental.print_receipt_btn')}</button> <button class="btn btn-small" style="background:var(--bg); color:var(--text); border:1px solid var(--border);" onclick="rentalUI.printBookingInvoice('${id}', ${!!isGroup})" title="${t('rental.print_invoice_title')}">${t('rental.print_invoice_btn')}</button> `;
}
function buildBookingPrintData(id, isGroup) {
    ...
    return {
        ...
        docLabel: t('rental.doc_label'), counterpartyLabel: t('rental.customer'),
...
function bookingGroupRowsHTML(entry) {
    ...
    const descCell = `<span style="cursor:pointer;" onclick="rentalUI.toggleBookingGroupExpand('${groupId}')">🧺 ${isExpanded ? '▾' : '▸'} ${tf('rental.items_count', rows.length)}</span>`;
    let html = `<tr>
...
function openBookingModal(prefillItemId, prefillDate) {
    ...
    <div class="modal-overlay active" onclick="rentalUI.closeBookingModal()">
        <div class="modal-box" onclick="event.stopPropagation()" style="max-width:460px; max-height:88vh; overflow-y:auto;">
            <h3 style="color:var(--primary); margin-bottom:14px;">${t('rental.new_booking')}</h3>
            ${bookingFormFieldsHTML(start, end)}
            <div style="display:flex; justify-content:flex-end; gap:8px;">
                <button class="btn btn-small" style="background:var(--bg); color:var(--text); border:1px solid var(--border);" onclick="rentalUI.closeBookingModal()">${t('common.cancel')}</button>
                <button class="btn btn-small btn-success" onclick="rentalUI.submitBookingForm()">${t('rental.create_booking')}</button>
            </div>
        </div>
    </div>`;
...
function bookingFormFieldsHTML(start, end) {
    return `
            <div class="form-group"><label>${t('rental.start_date')}</label><input type="date" id="rb-start" value="${start}" onchange="rentalUI.recalcBooking()"></div>
            <div class="form-group"><label>${t('rental.expected_return')}</label><input type="date" id="rb-end" value="${end}" onchange="rentalUI.recalcBooking()"></div>
            <div class="form-group"><label>${t('sale.customer_name')}</label><input type="text" id="rb-cust-name"></div>
            <div class="form-group"><label>${t('rental.customer_phone')}</label><input type="text" id="rb-cust-phone"></div>
            <div class="form-group"><label>${t('rental.customer_address')}</label><input type="text" id="rb-cust-address"></div>
            <div class="form-group">
                <label>${t('rental.add_asset')}</label>
                <input type="text" id="rb-add-item" list="rb-item-list" oninput="rentalUI.updateAddItemAvailability()" placeholder="${t('rental.type_to_filter_ph')}" style="min-width: 300px;" autocomplete="off">
                <datalist id="rb-item-list"></datalist>
                <div><small id="rb-add-avail" style="color:var(--text-light);"></small></div>
            </div>
            <div class="form-group"><label>${t('tr.quantity')}</label><input type="number" id="rb-add-qty" min="1" value="1"></div>
            <div class="form-group" style="padding-top: 22px;"><button type="button" class="btn btn-small" onclick="rentalUI.addCartLine()">${t('rental.add_plus')}</button></div>
            <div id="rb-cart" style="margin-bottom:14px; width:100%;"></div>

            <div class="form-group"><label>${t('rental.rate_type')}</label>
                <select id="rb-ratetype" onchange="rentalUI.recalcBooking()">
                    <option value="daily">${t('rental.rate.daily')}</option><option value="weekly">${t('rental.rate.weekly')}</option><option value="monthly">${t('rental.rate.monthly')}</option>
                </select>
            </div>
            <div class="form-group"><label>${t('rental.amount_etb')}</label><input type="number" id="rb-amount" step="0.01" min="0" oninput="rentalUI.syncPaidNow()"></div>
            <div class="form-group">
                <label>${t('rental.deposit_optional')} <span title="${t('rental.deposit_tooltip')}" style="cursor:help; color:var(--text-light);">ⓘ</span></label>
                <input type="number" id="rb-deposit" step="0.01" min="0" value="0">
            </div>
            <div class="form-group"><label>${t('rental.paid_now')}</label><input type="number" id="rb-paidnow" step="0.01" min="0" value="0"></div>
            <div class="form-group"><label>${t('rental.payment_channel')}</label>
...
function updateAddItemAvailability() {
    ...
    availEl.textContent = tf('rental.n_available', avail);
}
...
function renderCartLines() {
    ...
    if (bookingCart.length === 0) {
        el.innerHTML = `<p style="font-size:12px; color:var(--text-light); margin:0;">${t('sale.no_items_yet')}</p>`;
        return;
    }
    el.innerHTML = `<table style="margin:0;"><thead><tr><th>${t('rental.asset')}</th><th class="num">${t('tr.quantity')}</th><th style="text-align:center;">${t('rental.remove')}</th></tr></thead><tbody>
...
function addCartLine() {
    ...
    const item = itemByLabel(input.value);
    if (!item) { alert(t('rental.alert_pick_asset')); return; }
    const itemId = item.id;
    const qty = parseInt(qtyInput.value) || 1;
    const { start, end } = currentBookingWindow();
    const cap = end ? qtyAvailableForRange(item, start, end) : qtyAvailable(item);
    const already = bookingCart.filter(l => l.itemId === itemId).reduce((s, l) => s + l.qty, 0);
    if (already >= cap) { alert(tf('rental.alert_no_more_available', itemLabel(item))); return; }
...
function submitBookingForm() {
    ...
    if (bookingCart.length === 0) { errEl.innerText = t('rental.err_add_asset'); errEl.style.display = 'block'; return; }
    if (!custName) { errEl.innerText = t('rental.err_enter_customer_name'); errEl.style.display = 'block'; return; }
    if (!start || !end || end <= start) { errEl.innerText = t('rental.err_return_after_start'); errEl.style.display = 'block'; return; }
    for (const line of bookingCart) {
        const item = itemById(line.itemId);
        if (!item) { errEl.innerText = t('rental.err_asset_missing'); errEl.style.display = 'block'; return; }
        const cap = qtyAvailableForRange(item, start, end);
        if (line.qty > cap) { errEl.innerText = tf('rental.err_only_n_available', cap, itemLabel(item), fmtDate(start), fmtDate(end)); errEl.style.display = 'block'; return; }
    }
    if (paidNow > totalAmount + 0.001) { errEl.innerText = tf('rental.err_paid_now_exceeds', totalAmount.toFixed(2)); errEl.style.display = 'block'; return; }
    ...
    (async () => {
        ...
        addNotification(t('rental.notif_rental_booked_title'), tf('rental.notif_rental_booked_body', bookingCart.length, custName), tf('rental.notif_rental_booked_sub', fmtDate(start), fmtDate(end), totalAmount.toFixed(2)));
...
function checkOutBooking(id) {
    ...
    if (!b) return;
    const item = itemById(b.itemId);
    if (!confirm(tf('rental.confirm_checkout', b.qty, itemLabel(item), b.customerName))) return;
    b.status = 'out';
...
function cancelBooking(id) {
    const b = bookingById(id);
    if (!b) return;
    if (!confirm(t('rental.confirm_cancel_reservation'))) return;
    b.status = 'returned';
    b.damageNotes = (b.damageNotes ? b.damageNotes + ' | ' : '') + t('rental.note_cancelled_before_checkout');
...
function checkOutBookingGroup(groupId) {
    ...
    if (rows.length === 0) return;
    if (!confirm(tf('rental.confirm_checkout_group', rows.length, rows[0].customerName))) return;
    rows.forEach(b => {
        b.status = 'out';
        queueWrite({ kind: 'update', table: 'rentals', id: b.id, patch: { status: 'out' } });
    });
...
function cancelBookingGroup(groupId) {
    const rows = bookingGroupRows(groupId).filter(r => r.status === 'reserved');
    if (rows.length === 0) return;
    if (!confirm(t('rental.confirm_cancel_reservation_group'))) return;
    rows.forEach(b => {
        b.status = 'returned';
        b.damageNotes = (b.damageNotes ? b.damageNotes + ' | ' : '') + t('rental.note_cancelled_before_checkout');
        queueWrite({ kind: 'update', table: 'rentals', id: b.id, patch: { status: 'returned', damage_notes: b.damageNotes } });
...
function openCheckInModal(id) {
    ...
    <div class="modal-overlay active" onclick="rentalUI.closeCheckInModal()">
        <div class="modal-box" onclick="event.stopPropagation()" style="max-width:400px;">
            <h3 style="color:var(--primary); margin-bottom:14px;">${tf('rental.checkin_title', b.customerName)}</h3>
            <div class="form-group" style="margin-bottom:10px;"><label>${tf('rental.qty_returned_of', b.qty)}</label><input type="number" id="ci-qty-returned" min="0" max="${b.qty}" value="${b.qty}" style="width:100%;" oninput="rentalUI.updateCheckInMissingHint()"></div>
            <p id="ci-missing-hint" style="display:none; font-size:12px; color:var(--warning); margin:-4px 0 10px;"></p>
            <div class="form-group" style="margin-bottom:10px;"><label>${t('rental.condition_notes')}</label><input type="text" id="ci-notes" style="width:100%;" placeholder="${t('rental.condition_notes_ph')}"></div>
            ${b.depositAmount > 0 ? `
            <div class="form-group" style="margin-bottom:10px;"><label>${tf('rental.deposit_held', b.depositAmount.toFixed(2))}</label>
                <select id="ci-deposit-action" style="width:100%;" onchange="rentalUI.toggleForfeitField()">
                    <option value="refund">${t('rental.refund_full')}</option>
                    <option value="forfeit_partial">${t('rental.forfeit_part')}</option>
                    <option value="forfeit_full">${t('rental.forfeit_full')}</option>
                </select>
            </div>
            <div class="form-group" id="ci-forfeit-group" style="display:none; margin-bottom:10px;"><label>${t('rental.forfeit_amount')}</label><input type="number" id="ci-forfeit-amount" step="0.01" min="0" max="${b.depositAmount}" style="width:100%;"></div>
            <div class="form-group" style="margin-bottom:10px;"><label>${t('rental.refund_channel')}</label>
            ...
            <div style="display:flex; justify-content:flex-end; gap:8px;">
                <button class="btn btn-small" style="background:var(--bg); color:var(--text); border:1px solid var(--border);" onclick="rentalUI.closeCheckInModal()">${t('common.cancel')}</button>
                <button class="btn btn-small btn-success" onclick="rentalUI.submitCheckIn()">${t('rental.confirm_return')}</button>
            </div>
        </div>
    </div>`;
}
...
function updateCheckInMissingHint() {
    ...
    const item = itemById(b.itemId);
    const overdueNote = (isOverdue(b) && item && item.dailyRate) ? t('rental.missing_units_overdue_note') : '';
    hint.innerText = tf('rental.missing_units_hint', missing) + overdueNote;
...
function submitCheckIn() {
    ...
    if (b.depositAmount > 0) {
        ...
        if (action === 'forfeit_full') forfeited = b.depositAmount;
            ...
            if (forfeited > b.depositAmount + 0.001) { errEl.innerText = tf('rental.err_forfeit_exceeds', b.depositAmount.toFixed(2)); errEl.style.display = 'block'; return; }
        }
    ...
    b.damageNotes = notes + (writeOffResult && writeOffResult.consumedQty > 0
        ? `${notes ? ' | ' : ''}${tf('rental.note_units_not_returned', writeOffResult.consumedQty)}` + (writeOffResult.overdueCharge > 0 ? tf('rental.note_overdue_charge_added', writeOffResult.overdueCharge.toFixed(2)) : '') + '.'
        : '');
    ...
    addNotification(t('rental.notif_rental_returned_title'), tf('rental.notif_rental_returned_body', item ? item.desc : t('rental.asset'), b.customerName),
        writeOffResult && writeOffResult.consumedQty > 0 ? tf('rental.notif_units_not_returned_written_off', writeOffResult.consumedQty) : (notes || undefined));
    closeCheckInModal();
    renderBookingContent();
}
...
function openPaymentModal(id, isGroup) {
    ...
    if (isBranchClosed(branchId)) { paymentContext = null; alert(tf('rental.alert_branch_closed_payment', branchNameById(branchId))); return; }
    document.getElementById('sales-rentals-modals').innerHTML = `
    <div class="modal-overlay active" onclick="rentalUI.closePaymentModal()">
        <div class="modal-box" onclick="event.stopPropagation()" style="max-width:380px;">
            <h3 style="color:var(--primary); margin-bottom:8px;">${t('common.record_payment')}</h3>
            <p style="font-size:13px; color:var(--text-light); margin-bottom:14px;">${tf('rental.owed_of_total', balance.toFixed(2), total.toFixed(2))}</p>
            <div class="form-group" style="margin-bottom:14px;"><label>${t('rental.amount_etb')}</label><input type="number" id="rp-amount" step="0.01" min="0.01" max="${balance}" value="${balance.toFixed(2)}" style="width:100%;"></div>
            <div class="form-group" style="margin-bottom:14px;"><label>${t('sale.channel')}</label>
            ...
            <div style="display:flex; justify-content:flex-end; gap:8px;">
                <button class="btn btn-small" style="background:var(--bg); color:var(--text); border:1px solid var(--border);" onclick="rentalUI.closePaymentModal()">${t('common.cancel')}</button>
                <button class="btn btn-small btn-success" onclick="rentalUI.submitPayment()">${t('rental.save_payment')}</button>
            </div>
        </div>
    </div>`;
}
...
function submitPayment() {
    ...
    if (isNaN(amount) || amount <= 0) { errEl.innerText = t('rental.err_enter_valid_amount'); errEl.style.display = 'block'; return; }
    if (amount > balance + 0.001) { errEl.innerText = tf('rental.err_amount_exceeds_owed', balance.toFixed(2)); errEl.style.display = 'block'; return; }
